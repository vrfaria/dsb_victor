...
fatality_rate %>%
  ...
  ggplot(aes(x=fat_rate,y=age_group)) +
    ...
    geom_text(aes(label = round(fat_rate,0)), size = 3, vjust = 0, hjust = 1) +
...
fatality_year %>%
  ...
  ggplot(aes(x=case_month, y=fat_rate, color=age_group, group=age_group)) +
    geom_line() +
    facet_grid(icu_yn ~ sex, scales = "free_y") +
    labs(title = "Covid CFR by age group, sex and ICU admission") +
    geom_text(aes(label = round(fat_rate,0)), size = 2, vjust = 0, hjust = 0) +
